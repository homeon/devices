...
void setup(void) {
  ...
  if (ssid.length() > 2) {
    ...
    /* Se não for AP. checa se existe o endereco ativo no Firebase, caso contrario, cria */
    int ativo = Firebase.getBool(hard + "/ativo");
    Serial.println(ativo);

    if (!ativo) {

      /* Cria o Firebase com o padrao ligado */
      setaFirebase();

    } else {

      /* Se não criar, checa o último estado do Firebase no BOOT */
...
void loop(void) {
  ...
  /* Checa se esta em modo AP e o Status da tomada no Firebase   */
  if (!ap) {
